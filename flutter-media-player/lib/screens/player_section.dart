...
class _PlayerSectionState extends State<PlayerSection> {
  ...
  bool   _trackCompleteHandled = false;
  ...
  void initState() {
    ...
    player.addListener(_onPlaybackUpdate);
  ...
  void dispose() {
    ...
    player.removeListener(_onPlaybackUpdate);
    super.dispose();
  }
  ...
  void _onPlaybackUpdate() {
    ...
    if (!_trackCompleteHandled
      ...
      if (pid != null) {
        ...
        pl.playCounts[current] = (pl.playCounts[current] ?? 0) + 1;
        repo.savePlaylists();
      }
  ...
  Widget build(BuildContext context) {
    ...
    final titleText = player.currentPath != null
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        IconButton(
          icon: SvgPicture.asset(
            player.repeatEnabled
                ? 'assets/icons/ph--repeat-bold.svg'
                : 'assets/icons/ph--repeat-off-bold.svg',
            color: player.repeatEnabled ? brightGreen : darkGreen,
            width: 24,
            height: 24,
          ),
          onPressed: () => context.read<PlayerModel>().toggleRepeat(),
        ),
        const SizedBox(width: 24),
        // Volume
        Transform.translate(
