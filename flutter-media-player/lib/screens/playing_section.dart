...
class _PlayingSectionState extends State<PlayingSection> {
  ...
  Widget build(BuildContext context) {
    ...
    final selectedPlaylist = repo.selectedPlaylistId != null
      ? repo.playlists.firstWhere(
          (p) => p.id == repo.selectedPlaylistId,
          orElse: () => Playlist(id: '', name: ''),
        )
      : null;
    final songs = selectedPlaylist?.songPaths ?? [];
      ...
      onDragDone: (details) {
        ...
        setState(() {
          if (selectedPlaylist != null && selectedPlaylist.id.isNotEmpty) {
            selectedPlaylist.songPaths.addAll(paths);
            repo.savePlaylists();
          } else {
                            ...
                            trailing: isHover
                              ? InkWell(
                                  onTap: () {
                                    setState(() {
                                      if (songs.isNotEmpty) {
                                        selectedPlaylist!.songPaths.removeAt(i);
                                        repo.savePlaylists();
                                        if (track == current) {
